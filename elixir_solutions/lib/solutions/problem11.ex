defmodule Problem11 do
  ...
  defp grid_fetch(m, n) do
    Enum.at(@grid, m) |> Enum.at(n)
  end
  ...
  defp max_search(m, n) do
    ...
    todo =
      cond do
        m <= 16 and n >= 3 and n <= 16 ->
          [:right, :down, :down_right, :down_left]
    ...
    Enum.map(todo, fn x ->
      case x do
        :right ->
          grid_fetch(m, n) * grid_fetch(m, n + 1) * grid_fetch(m, n + 2) * grid_fetch(m, n + 3)

        :down ->
          grid_fetch(m, n) * grid_fetch(m + 1, n) * grid_fetch(m + 2, n) * grid_fetch(m + 3, n)

        :down_right ->
          grid_fetch(m, n) * grid_fetch(m + 1, n + 1) * grid_fetch(m + 2, n + 2) *
            grid_fetch(m + 3, n + 3)
    ...
    # |> IO.inspect(label: "Products of (#{m}, #{n})")
    |> Enum.max(&>=/2, fn -> 0 end)
  end
  ...
  def solution(m, n, result),
    do: solution(m, n + 1, List.update_at(result, m, &(&1 ++ [max_search(m, n)])))
end
